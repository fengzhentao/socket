...
public class TCPServer {
    ...
    // 将给定的消息转发给所有客户端(不包括自己)
    private synchronized void sendToAllNoMe(String name, String message) {
        for (String key : storeInfo.keySet()) {
            if (!key.equals(name)) {
                storeInfo.get(key).println(message);
            }
        }
    }
...
//                sendToAll("[系统通知] “" + name + "”已上线");
                sendToAllNoMe(name, "[系统通知] “" + name + "”已上线");
...
//                    sendToAll(name+"："+ msgString);
                    sendToAllNoMe(name, name+"："+ msgString);
                }
            } catch (Exception e) {
                // e.printStackTrace();
            } finally {
...
//                sendToAll("[系统通知] "+name + "已经下线了。");
                sendToAllNoMe(name, "[系统通知] "+name + "已经下线了。");
